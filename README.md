...
Finderから起動したい場合は、`Agent Monitor.command` をダブルクリックしてください。  
サーバーを起動して、自動でブラウザを開きます。

## Macで継続して見れるようにする

ブラウザのURLは `http://localhost:8799/` ですが、裏側のサーバーが止まると表示できません。  
Macのログイン時に自動起動したい場合は、LaunchAgentを使います。
...
これで自動起動用の本体が `~/.local/share/codex-agent-monitor` に入り、ログイン時に自動起動し、落ちても再起動されます。  
GitHubから更新した後は、同じコマンドをもう一度実行すると自動起動側にも反映されます。
